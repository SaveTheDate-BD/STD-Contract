...
library DateUtils {
   ...
   */
    ...
    function convertSliceToUint16(strings.slice s)
        private
        pure
        returns (uint16)
    {
    ...
    function splitTimeSlice(string _time)
        private
        pure
        returns (strings.slice[3])
    {
        strings.slice[3] memory sArr;
   ...
   */
    function isISOformat(string _dt) internal pure returns (bool) {
        ...
        if (strings.count(sDate, sHyphen) != 2) {
            return false;
        }
    ...
    function isTimeFormat(string _time) private pure returns (bool) {
        ...
        if (strings.count(sTime, sColon) != 2) {
            return false;
        }
   ...
   */
    function isISOtimeFormat(string _dt) internal pure returns (bool) {
        ...
        if (!isISOformat(sISO.toString())) {
            return false;
        }
        if (!isTimeFormat(sDate.toString())) {
            return false;
        }
   ...
   */
    ...
    function incrementLeapYear(
        uint16 _year,
        uint16 _yearCounter,
        uint256 _ts
    ) private pure returns (uint16 yearCounter, uint256 ts) {
    ...
    function addYearSeconds(uint256 _ts, uint16 _year)
        private
        pure
        returns (uint256)
    {
        ...
        if (_year < 1972) {
            ...
            if (_year >= 2000) {
                ...
                (yearCounter, ts) = incrementYearAndTimestamp(
                    _year,
                    yearCounter,
                    ts,
                    400,
                    SECONDS_IN_400_YEARS
                );
    ...
    function addMonthSeconds(uint16 _year, uint8 _month)
        private
        pure
        returns (uint256)
    {
    ...
    function YMDuintToBytes(
        uint16 _y,
        uint8 _m,
        uint8 _d
    ) private pure returns (strings.slice[]) {
   ...
   */
    function convertTimestampToDateString(uint256 _dt)
        internal
        pure
        returns (string)
    {
   ...
   */
    function convertTimestampToDateTimeString(uint256 _dt)
        internal
        pure
        returns (string)
    {
        ...
        (year, month, day, hour, minute, second) = convertTimestampToYMDHMS(
            _dt
        );
        return
            convertYMDHMStoDateTimeString(
                year,
                month,
                day,
                hour,
   ...
   */
    function convertDateStringToTimestamp(string _dt)
        internal
        pure
        returns (uint256)
    {
    ...
    @return Returns timestamp as `uint256`
   */
    function convertDateTimeStringToTimestamp(string _dt)
        internal
        pure
        returns (uint256)
    {
        ...
        (
            ...
            minute,
            second
        ) = convertDateTimeStringToYMDHMS(_dt);
