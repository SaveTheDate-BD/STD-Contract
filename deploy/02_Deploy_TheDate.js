...
module.exports = async ({ getNamedAccounts, deployments, getChainId }) => {
  ...
  log("----------------------------------------------------");
  const Contract = await deploy("SaveTheDate", {
    from: deployer,
    args: args,
    log: true,
  });
  log(`You have deployed an NFT contract to ${Contract.address}`);
  ...
  const Factory = await ethers.getContractFactory("SaveTheDate");
